...
final class UserProfile {
    ...
    func displayName(language: AppLanguage) -> String {
        ...
        if trimmed.isEmpty || Self.localizedDefaultNames.contains(trimmed) {
            return AppText.text(.defaultProfileName, language: language)
        }
        return trimmed
    }
    ...
    private static var localizedDefaultNames: Set<String> {
        Set(AppLanguage.allCases.map { AppText.text(.defaultProfileName, language: $0) })
    }
