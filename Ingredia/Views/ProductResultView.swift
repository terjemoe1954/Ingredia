...
struct ProductResultView: View {
    ...
    private var productAssessment: ProductSafetyAssessment {
        AlternativeRankingService.assessment(for: product, profile: profile, language: language)
    }
            ...
            List {
                ...
                Section(AppText.text(.betterAlternatives, language: language)) {
                    if isLoadingAlternatives {
                        ...
                        default:
                            infoMessage(alternativeOutcome.message(language: language))
                        }
                    }
                }
    ...
    private func persistViewedAlternative(_ product: ScannedProduct) -> ScannedProduct {
        ...
        if let existing = try? modelContext.fetch(descriptor).first {
            existing.updateProductData(from: product)
            try? modelContext.save()
            return existing
        }

        let savedProduct = product.copyForStorage()
    ...
    private var alternativesTaskID: String {
        "\(product.barcode)-\(language.rawValue)"
    }
    ...
    private var productHeroCard: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                if product.hasMergedSourceProviders {
                    Text(AppText.text(.dataSourceMergedDescription, language: language))
                        .font(.caption2)
                        .foregroundStyle(.secondary)
                }
            }
    ...
    private var dataQualityCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text(productAssessment.dataQuality.title(language: language))
                .font(.headline)

            Text(productAssessment.dataQuality.detail(language: language))
                .font(.subheadline)
                .foregroundStyle(.secondary)
        ...
        .accessibilityLabel(
            [
                ...
                productAssessment.dataQuality.title(language: language),
                productAssessment.dataQuality.detail(language: language),
...
private struct AlternativeRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack(alignment: .top, spacing: 12) {
                VStack(alignment: .leading, spacing: 4) {
                    Text(alternative.product.displayName(language: language))
                        .font(.headline)
            ...
            if let topFinding = alternative.assessment.result.findings.first {
                ...
                Text(AppText.text(.noRegisteredConflictsInAvailableData, language: language))
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
            }

            Text(
                alternative.assessment.dataQuality.title(language: language)
                + " • "
                + alternative.assessment.dataQuality.detail(language: language)
            )
                .font(.caption)
            ...
            HStack(spacing: 8) {
                ...
                SourceTrustBadge(trustLevel: alternative.product.sourceTrustLevel, language: language)
            }
        ...
        .accessibilityLabel(
            [
                alternative.product.displayName(language: language),
                ...
                "\(AppText.text(.accessibilityStatus, language: language)): \(alternative.assessment.result.level.title(language: language))",
                alternative.assessment.dataQuality.title(language: language),
                alternative.recommendationReasons
                    .map { AppText.text($0, language: language) }
                    .joined(separator: ", ")
    ...
    private var safetyChip: some View {
        HStack(spacing: 6) {
            ...
            Text(alternative.assessment.result.level.title(language: language))
                .font(.caption.weight(.semibold))
                .lineLimit(1)
        }
    ...
    private var sourceText: String {
        alternative.isFromHistory
        ? AppText.text(.alternativeSourceHistory, language: language)
        : alternative.product.sourceProviderName
    }
}
