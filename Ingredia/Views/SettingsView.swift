...
struct SettingsView: View {
    ...
    var body: some View {
        List {
            ...
            Section(AppText.text(.dataSources, language: language)) {
                ...
                ForEach(ProductLookupAggregator.sourceStatuses()) { status in
    ...
    private func dataSourceRow(_ status: ProductDataSourceStatus) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack {
                ...
                Text(
                    status.isEnabled
                    ? AppText.text(.dataSourceActive, language: language)
                    : AppText.text(.dataSourceInactive, language: language)
                )
            ...
            HStack(spacing: 8) {
                ...
                SourceTrustBadge(trustLevel: status.trustLevel, language: language)
            }

            if !status.isEnabled {
                Text(AppText.text(.dataSourceInactiveDescription, language: language))
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
